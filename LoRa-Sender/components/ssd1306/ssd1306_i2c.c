...
{
	/*
	 * Reset the display BEFORE creating the I2C bus.
	 * If a previous boot ended with a stack overflow in the middle of an I2C
	 * transaction, the SSD1306 may be holding SDA low indefinitely. A hardware
	 * reset clears that state before the I2C peripheral is initialized.
	 */
	if (reset >= 0) {
	...
	/* Reset the bus state (clocks out any stuck slave byte). */
...
void i2c_display_image(SSD1306_t * dev, int page, int seg, uint8_t * images, int width)
{
	...
	/*
	 * Transação 2: envia byte de controle + pixels em um único frame I2C
	 * sem alocar buffer extra na stack (original passava ponteiro direto).
	 */
