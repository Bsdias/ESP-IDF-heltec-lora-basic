...
void config_i2c_interface(){ //Config Display
	...
	ssd1306_display_text(&dev, 1, " Kit-01[Node 1] ", 18, true);
  	// CONFIG_SSD1306_128x64
}
...
void task_tx(void *p)
{
   ...
   while(1) {
      ...
      i++;
      sprintf(mm_rain, "%d", i);
	   strcpy(data_packet, data_rain);
	   strcat(data_packet, mm_rain);
      //strcat(data_packet, "mm");

      vTaskDelay(pdMS_TO_TICKS(2000));
      lora_send_packet((uint8_t*)data_packet, 20);
      printf("Enviado: %d\n",i);
      show_RSSI_and_SNR();
      ssd1306_display_text(&dev, 4, "Dados enviados",20, false);
      ssd1306_display_text(&dev, 5, data_packet,20, false);
   }
}
